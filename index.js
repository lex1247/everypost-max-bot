...
async function contentState(channelId,userId,input) {
 ...
 const rows=(await pool.query(`SELECT c.id,c.provider,c.remote_id,c.canonical_url,c.original_url,c.author,c.title,c.published_at,c.duration,
  c.metrics,c.metadata_status,c.state,c.due_at,c.post_id,c.last_error,q.revision AS queue_revision,q.status AS queue_status,q.due_at AS queue_due_at,p.published_mid
...
for(const action of ['channels','state','sources','seed','decide','toggle','hair-policy','move'])app.post('/content/api/'+action,contentEndpoint(action));
