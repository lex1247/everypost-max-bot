...
app.get("/", (req, res) => res.status(ready ? 200 : 503).json({
  service: "EveryPost MAX", version: VERSION, content_recovery: 2, content_review: 1, status: ready ? "running" : "starting"
}));
...
async function contentQueue(row,prepared) {
 ...
 const client=await pool.connect();try{await client.query('BEGIN');
  ...
  // Preparation can finish after an outage. Past slots require an explicit new date.
  const missed=new Date(fresh.due_at).getTime()<=Date.now();
  const timingError=missed?'Видео подготовлено, но выбранное время прошло. Назначьте новую дату публикации.':null;
  await client.query(`INSERT INTO ep_schedules(post_id,due_at,timezone,scheduled_by,access_version,body_snapshot,status,last_error)
   VALUES($1,$2,$3,$4,$5,$6::jsonb,$7,$8)`,[p.id,fresh.due_at,access.channel.timezone||'Europe/Moscow',row.selected_by,access.version,JSON.stringify(body),missed?'paused':'scheduled',timingError]);
  await client.query(`UPDATE ep_content_candidates SET state='queued',post_id=$2,prepared_body=$3::jsonb,content_hash=$4,
   lease_until=NULL,last_error=$6,scan_state='ready',scan_hash=$4,fingerprint=$5::jsonb,updated_at=NOW() WHERE id=$1`,[row.id,p.id,JSON.stringify(base),prepared.content_hash,JSON.stringify(prepared.fingerprint||null),timingError]);
  await client.query('COMMIT');
 }catch(e){await client.query('ROLLBACK');throw e;}finally{client.release();}
}
...
async function contentPreparationFailed(row,e){
 const attempts=Number(row.prepare_attempts||0),retry=e.safeRetry===true&&attempts<24;
 const delay=Math.min(1800,60*Math.pow(2,Math.min(attempts,5)));
 const message=retry?'Сервис подготовки временно недоступен. Повтор через '+Math.ceil(delay/60)+' мин. Выбор и время сохранены. '+String(e.message):String(e.message);
 const changed=await pool.query(`UPDATE ep_content_candidates SET state=$2,last_error=$3,lease_until=NULL,
  prepare_attempts=prepare_attempts+1,prepare_next_at=NOW()+($4 * INTERVAL '1 second'),updated_at=NOW()
  WHERE id=$1 AND post_id IS NULL AND state='preparing'`,
  [row.id,retry?'selected':'failed',message.slice(0,500),delay]);
 if(!retry&&changed.rowCount)await notify(row.selected_by,'⚠️ Подготовка ролика остановлена: '+String(e.message).slice(0,300)+'\nВыбор и дата сохранены. Откройте «Контент → Нужна проверка».');
}
async function contentInspectNext(existingOnly=false){
 ...
 try{
  ...
  console.log('CONTENT INSPECTION DONE:',scan.id);
   ...
   scan_next_at=NOW()+($4 * INTERVAL '1 second') WHERE id=$1`,[scan.id,e.safeRetry&&scan.scan_attempts<24?'retry':'failed',String(e.message).slice(0,500),Math.min(1800,60*2**Math.min(scan.scan_attempts,5))]);}
 return true;
}
...
async function contentRefresh(channelId,userId){
 await contentAccess(channelId,userId,true);
 const client=await pool.connect();try{await client.query('BEGIN');
  await client.query('SELECT id FROM channels WHERE id=$1 FOR UPDATE',[channelId]);
  const sources=await client.query(`UPDATE ep_content_sources SET next_at=NOW()
   WHERE channel_id=$1 AND enabled AND next_at>NOW() AND (checked_at IS NULL OR checked_at<NOW()-INTERVAL '5 minutes') RETURNING id`,[channelId]);
  const inspections=await client.query(`UPDATE ep_content_candidates SET scan_state='retry',scan_attempts=0,scan_next_at=NOW(),scan_error=NULL
   WHERE channel_id=$1 AND state='new' AND post_id IS NULL AND selected_by IS NULL AND scan_state='failed'
   AND (scan_error LIKE 'Обработчик %' OR scan_error LIKE 'Сервис обработки временно недоступен.%' OR scan_error LIKE 'Сервис обработки запускается.%' OR scan_error LIKE 'Сервис чтения не ответил.%')
   AND (hair_relevance='match' OR NOT COALESCE((SELECT hair_only FROM ep_content_preferences WHERE channel_id=$1),FALSE)) RETURNING id`,[channelId]);
  await audit(channelId,userId,'content_refresh',null,{sources:sources.rowCount,inspections:inspections.rowCount},client);
  await client.query('COMMIT');return {sources:sources.rowCount,inspections:inspections.rowCount};
 }catch(e){await client.query('ROLLBACK');throw e;}finally{client.release();}
}
async function contentState(channelId,userId,input) {
 ...
 const rows=(await pool.query(`SELECT c.id,c.provider,c.remote_id,c.canonical_url,c.original_url,c.author,c.title,c.published_at,c.duration,
  ...
  WHERE c.channel_id=$1 AND ((c.state=$2 AND ($2<>'new' OR (c.scan_state='ready' AND (NOT $4 OR c.hair_relevance='match'))) AND ($2<>'failed' OR c.scan_state<>'duplicate'))
   ...
   OR ($2='duplicates' AND c.state<>'skipped' AND c.scan_state='duplicate' AND c.post_id IS NULL))
 ...
 const summary=(await pool.query(`SELECT COUNT(*) FILTER(WHERE scan_state='ready') AS ready, COUNT(*) FILTER(WHERE scan_state IN ('pending','retry')) AS checking, COUNT(*) FILTER(WHERE scan_state='failed') AS failed FROM ep_content_candidates WHERE channel_id=$1 AND state='new' AND (NOT $2 OR hair_relevance='match')`,[channelId,hairOnly])).rows[0];
 return {ok:true,summary,hairOnly,title:a.channel.title,timezone:a.channel.timezone||'Europe/Moscow',owner:a.owner,items:rows.slice(0,12),hasMore:rows.length>12,sources};
}
...
function contentEndpoint(action) {
 return async(req,res)=>{calendarHeaders(res);try{
  ...
  else if(action==='refresh')Object.assign(result,await contentRefresh(channelId,auth.userId));
...
for(const action of ['channels','state','sources','seed','decide','toggle','hair-policy','move','replace','recover','refresh'])app.post('/content/api/'+action,contentEndpoint(action));
